...
namespace AwoBot.AudioCore.Utils
{
  public static class Bytes
  {
    ...
    public static ByteSizeFactor ByteFact { get; private set; } = new ByteSizeFactor("Byte", "b", 1L);
    ...
    public static ByteSizeFactor[] Factors { get; private set; } = new[] { ByteFact, KiloFact, MegaFact, GigaFact, TeraFact, PetaFact };

    public static long Byte(double value) => ByteFact.Convert(value);
    ...
    {
      var factor = Factors.OrderBy(x => x.Factor).FirstOrDefault(x => (x.Factor * 1024) > value, ByteFact);
      return (((double)value) / factor.Factor).ToString(format) + factor.ShortHand;
    }
  }
}
